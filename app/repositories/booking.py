...
class BookingRepository(BaseRepository[Booking]):
    ...
    async def get_by_room_id(self, room_id: int) -> list[Booking]:
        result = await self.session.execute(select(Booking).where(Booking.room_id == room_id))
        return result.scalars().all()
    
    async def get_active_bookings(self) -> list[Booking]:
        result = await self.session.execute(select(Booking).where(Booking.status != BookingStatus.cancelled))
        return result.scalars().all()
